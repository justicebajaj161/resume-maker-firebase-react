...
describe('Signin Component', () => {
  ...
  it('uses AuthContext', () => {
      // Check if the setIsAuthenticated function has been exposed to the window object
      cy.window().its('setIsAuthenticated').should('be.a', 'function');

      // Optionally, you can call this function to ensure it behaves as expected
      // cy.window().invoke('setIsAuthenticated', true);
      // cy.window().its('Cypress').invoke('log', 'AuthContext is in use');
  });
});
...
describe('Responsive Navbar', () => {
  ...
  it.only('checks navbar in desktop view', () => {
  ...
  it.only('checks navbar in mobile view', () => {
    cy.viewport(375, 667);
